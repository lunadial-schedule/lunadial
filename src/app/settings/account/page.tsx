 ...
 * 계정 설정 페이지
 *
 * 즐겨찾기 현황, 치지직 계정 연동/해제, 계정 삭제를 관리한다.
 */
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <Label>등록된 스트리머</Label>
              <span className="text-sm">
                <span className="font-medium text-foreground">{favoritesCount}</span>
                <span className="text-muted-foreground"> / {MAX_FAVORITES}</span>
              </span>
            </div>
            <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
              <div 
                className="h-full bg-primary" 
                style={{ width: `${Math.min((favoritesCount / MAX_FAVORITES) * 100, 100)}%` }} 
              />
            </div>
