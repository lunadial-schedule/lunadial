...
-- 2. schedule_revisions.edited_by FK → ON DELETE SET NULL
ALTER TABLE public.schedule_revisions
  DROP CONSTRAINT IF EXISTS schedule_revisions_edited_by_fkey;
ALTER TABLE public.schedule_revisions
  ADD CONSTRAINT schedule_revisions_edited_by_fkey
  FOREIGN KEY (edited_by) REFERENCES public.users(id) ON DELETE SET NULL;

-- 3. schedule_update_logs.actor_user_id FK → ON DELETE SET NULL
